              ...
              {viewDoc?.files && viewDoc.files.length > 0 ? (
                <div className="space-y-2">
                  ...
                  {viewDoc.files.map((file) => (
                    <div key={file.id} className="bg-card rounded-xl p-3 flex items-center gap-3 border border-border">
                      {file.type === "image" ? (
        ...
        <Drawer open={showAddList} onOpenChange={setShowAddList}>
          <DrawerContent dir="rtl">
            <DrawerHeader className="text-right">
              ...
              <DrawerDescription>أنشئ قائمة عائلية أو شخصية</DrawerDescription>
            </DrawerHeader>
